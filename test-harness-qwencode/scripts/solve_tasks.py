...
solve_tasks.py
--------------
Run this from INSIDE Claude Code (backed by Qwen Coder).
...
def main():
    ...
    if args.list:
        for t in tasks:
            ...
            status = '[X]' if (SOLUTIONS_DIR / f"{t['id']}.ts").exists() else '[ ]'
        print(f"  {status} {t['id']:<16} [{t['category']}]  {t['title']}")
        return
    ...
    print(f"# {'='*60}")
    print(f"# QWENCODE EVAL — SOLVE PHASE")
